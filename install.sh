...
if (( IS_MACOS )); then
  if [[ -z "$WANT_BREW" ]]; then
    if (( NO_PROMPT )) || confirm "Run brew sync for host '$HOST'?"; then
      WANT_BREW=1
    else
      WANT_BREW=0
    fi
  ...
  if (( WANT_BREW )); then
    say "${c_dim}── brew sync ──${c_off}"
    "$DOTFILES_DIR/scripts/sync-brewfile.zsh" </dev/tty
  else
    info "skipping brew sync"
  fi
else
  info "skipping brew sync (non-macOS)"
fi
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if stow -v -d "$DOTFILES_DIR" -t "$HOME" --no-folding "$pkg" 2>&1 | sed "s/^/  /"; then
    ok "stowed $pkg"
  else
...
# Run config sync to commit any changes
say "${c_dim}── config sync ──${c_off}"
"$DOTFILES_DIR/scripts/sync.sh" --yes </dev/null
