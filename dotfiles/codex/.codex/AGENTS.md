...
- Do not hardcode stale frontier-model overrides for Codex native child agents. If an explicit frontier override is necessary, use the current frontier default from `OMX_DEFAULT_FRONTIER_MODEL` / the repo model contract (currently `gpt-5.6-sol`), not older values such as `gpt-5.5`.
...
| Spark (explorer/fast) | `gpt-5.6-luna` | low | Fast triage, repository exploration, lightweight synthesis, and low-latency routing. |
| Standard (subagent default) | `gpt-5.6-terra` | medium | Default capability for installable specialists and general worker agents. |
| `explore` | `gpt-5.6-luna` | low | Fast codebase search, symbol lookup, dependency tracing, and repository mapping. |
| `analyst` | `gpt-5.6-sol` | medium | Requirements analysis, hidden constraints, acceptance criteria, feasibility assessment. |
| `planner` | `gpt-5.6-sol` | medium | Task decomposition, sequencing, execution plans, dependency analysis, and risk identification. |
| `architect` | `gpt-5.6-sol` | high | System architecture, API boundaries, long-term maintainability, and technical trade-offs. |
| `debugger` | `gpt-5.6-sol` | high | Root-cause analysis, regression isolation, performance bottlenecks, and difficult debugging. |
| `executor` | `gpt-5.6-terra` | medium | General implementation, feature development, refactoring, and code modifications. |
| `team-executor` | `gpt-5.6-sol` | medium | Coordinates larger implementation efforts involving multiple worker agents. |
| `verifier` | `gpt-5.6-sol` | high | Verifies correctness, validates claims, checks completion criteria, and evaluates test adequacy. |
| `code-reviewer` | `gpt-5.6-sol` | high | Comprehensive review covering correctness, maintainability, performance, security, and style. |
| `dependency-expert` | `gpt-5.6-sol` | high | Evaluates libraries, SDKs, APIs, licensing, compatibility, and migration risks. |
| `test-engineer` | `gpt-5.6-terra` | medium | Designs test strategy, improves coverage, writes tests, and hardens flaky test suites. |
| `designer` | `gpt-5.6-sol` | high | UX architecture, interaction flows, information architecture, and design consistency. |
| `writer` | `gpt-5.6-terra` | medium | Documentation, migration guides, changelogs, README improvements, and developer guidance. |
| `git-master` | `gpt-5.6-terra` | medium | Commit organization, rebasing, branch management, merge conflict resolution, and history cleanup. |
| `code-simplifier` | `gpt-5.6-sol` | high | Simplifies implementations while preserving behavior, improving readability and maintainability. |
| `researcher` | `gpt-5.6-luna` | low | Searches documentation, existing code, examples, specifications, and summarizes findings quickly. |
| `critic` | `gpt-5.6-sol` | high | Challenges assumptions, identifies weaknesses, evaluates alternatives, and stress-tests plans. |
| `vision` | `gpt-5.6-sol` | low | Analyzes screenshots, diagrams, UI mockups, wireframes, and visual debugging artifacts. |
<!-- OMX:MODELS:END -->
...
OMX persists runtime state under `.omx/` — but ONLY inside a git repository. Before creating any `.omx/` file, verify `git rev-parse --is-inside-work-tree` succeeds; in a non-git directory do NOT create `.omx/` (no state, notepad, memory, plans, or logs).
