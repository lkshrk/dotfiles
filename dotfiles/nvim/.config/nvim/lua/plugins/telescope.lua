return {
  ...
  config = function()
    ...
    local function git_hl_for_status(status)
      if status:match '%?' then return 'GitSignsAdd' end -- untracked
      if status:match 'A' then return 'GitSignsAdd' end -- staged add
      if status:match 'D' then return 'GitSignsDelete' end -- deleted
      return 'GitSignsChange' -- modified, renamed, etc.
    end
    ...
    -- Needs CSI-u/kitty keyboard protocol (ghostty; tmux with extended-keys on).
    vim.keymap.set('n', '<C-S-f>', builtin.live_grep, { desc = 'Search file contents' })
